...
internal suspend fun synthesizeInlineVoiceMessage(
    ...
    val response = TtsSynthesizer(koin.get<TTSManager>()).synthesize(
        setting = provider,
        chunk = TtsChunk(index = 0, text = cleanText),
    )
    ...
    val extension = if (format == AudioFormat.PCM) "wav" else format.name.lowercase()
    val target = File(directory, "${UUID.randomUUID()}.$extension")
    val playableBytes = if (format == AudioFormat.PCM) {
        pcm16MonoToWav(
    ...
    pruneInlineVoiceCache(directory)

    val durationMs = response.duration
        ?.takeIf { it > 0f }
        ?.let { (it * 1_000f).toLong() }
        ?: ((cleanText.length / 5.0).coerceAtLeast(1.0) * 1_000.0).toLong()
