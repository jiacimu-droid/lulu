...
object ExamStudyPlan {
    ...
    val dailyPlans: Map<LocalDate, DailyStudyPlan> = listOf(
        ...
        daily("2026-07-03", "慢启动但能学：法理2 + 刑法1入口", "起床后先吃清淡、喝热水、洗漱，整理桌面 10 分钟；今天不追上午时长", "法理学第 2 章：回看错题和目录框架 30-40 分钟，不背完整表述、不补难题", "刑法学第 1 章：听众合法硕刑法课程 30-40 分钟，只抓犯罪论总入口", "不背单词 120 个（6 组）：拆成 3 次完成，每次 2 组，中间必须离桌休息", "饭后散步或慢走 15-20 分钟，等困劲过去再回桌面", "睡前 10 分钟写明天第一步，23:30 前结束学习入口"),
        daily("2026-07-04", "恢复后轻推进：刑法1 + 法理2封口", "上午先观察身体状态，热水、清淡早饭、整理桌面 10 分钟后再开始", "刑法学第 1 章：听众合法硕刑法课程 40-50 分钟，抓犯罪论总入口和构成要件", "刑法学第 1 章：做 5-8 题并标错因，不追求题量", "法理学第 2 章：用 15 分钟口头复述框架，卡住就看目录提示", dailyVocabularyTaskTitle, "晚上只做轻收尾和提前睡；如果白天状态好，再加长难句 1 句"),
    ...
    fun todaySchedule(date: LocalDate = LocalDate.now()): List<StudyScheduleBlock> {
        ...
        if (date == LocalDate.of(2026, 7, 3)) {
            return listOf(
                ...
                StudyScheduleBlock("15:15-15:55", "刑法入口", "听众合法硕刑法课程第 1 章，只抓犯罪论总入口，听完写 3 个关键词。"),
