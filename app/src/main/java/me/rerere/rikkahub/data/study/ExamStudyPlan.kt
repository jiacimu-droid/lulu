...
object ExamStudyPlan {
    ...
    val studyHabitReference = """
        ...
        - 排刑法、民法新章时，默认先拆成 60-90 分钟听课切片 + 已学内容小测/小框架；整章听完后，再安排章节题 + 错题/框架 + 第一轮背诵。恢复日可缩成 30-60 分钟入口切片，但不要把一章压成一个短任务。
        - 未听完整章前不要安排整章章节题，也不要机械写“做 5 道题”。半章学习后只做已学内容小测、例题回忆、关键词复述或小框架；等整章课程闭环后，再集中做章节题、错题整理和第一轮背诵。
        - 法硕专业课必须按倍数估算：听课一轮只是第一层成本，后面还有做题、错题总结、框架和三轮背诵。正常学习日不能只排“听一小段课”；除身体恢复日外，至少要有 60-90 分钟主块、30-45 分钟已学小测/题目/错题块、15-25 分钟背诵或框架封口。
        - 法理学不重听大课，但不是只看目录；7 月先做目录树、错题回看和第一轮关键词背诵，后面还要二轮规范表述、三轮限时输出。
    ...
    val monthlyPlans = listOf(
        MonthlyStudyPlan(
            ...
            listOf(
                ...
                "宪法学、法制史本月只做第 1 章入门框架和少量已学小测，目标是建立科目感觉",
        ...
        MonthlyStudyPlan(
            ...
            listOf(
                "续接 7 月实际进度，刑法、民法按章推进：整章听完后才安排章节题、错题和框架，不做半章硬刷题",
                "刑法目标按真实课时推进到中段；民法推进到前中段。若落后，先保刑民核心章节闭环，不用硬追数字",
                "宪法学、法制史进入稳定入门：每周 2-3 次小块听课/框架/已学小测，避免 9 月突然开荒",
                "第一轮背诵成为日常：每天至少 20-30 分钟目录式回忆或关键词输出，法理完整走完一轮",
    ...
    val julyWeeks = listOf(
        WeeklyStudyPlan(
            ...
            tasks = listOf(
                ...
                "刑法第 1 章没听完整前不做整章章节题，只做已学内容小测、例题回忆、关键词和不懂点",
                "法理第 1-3 章：错题回看、目录式背诵和框架补洞，不重复从零听课，但必须开第一轮背诵",
        ...
        WeeklyStudyPlan(
            ...
            tasks = listOf(
                ...
                "刑法第 2 章启动：累计 2-3 个听课切片，只做已学内容小测，不要求一周内完美吃透",
        ...
        WeeklyStudyPlan(
            ...
            tasks = listOf(
                ...
                "本周开始固定错题本：只整理已闭环章节的错因，不从没学完的整章题里硬抽题",
        ...
        WeeklyStudyPlan(
            ...
            tasks = listOf(
                ...
                "刑法第 3 章、民法第 3 章按课时切片推进；能闭环的章节当天补章节题和框架，没闭环的只做已学小测",
    ...
    val dailyPlans: Map<LocalDate, DailyStudyPlan> = listOf(
        ...
        daily("2026-07-03", "病后慢启动：保底 + 可加码", "起床后先吃清淡、喝热水、洗漱，整理桌面 10 分钟；今天不追上午时长", "法理学第 2 章：回看错题和目录框架 30-40 分钟，不背完整表述、不补难题", "刑法学第 1 章：听众合法硕刑法课程 30-40 分钟，作为 6 小时课程的恢复日入口切片，听完写 3 个关键词和 1 个不懂点", "加码任务：如果晚上身体还可以，只做刑法第 1 章已学内容小测和例题回忆，不做整章章节题；不舒服就只保留保底", "不背单词 120 个（6 组）：拆成 3 次完成，每次 2 组，中间必须离桌休息", "饭后散步或慢走 15-20 分钟，等困劲过去再回桌面", "睡前 10 分钟写明天第一步，23:30 前结束学习入口"),
        daily("2026-07-04", "恢复到正常最小强度：刑法1主块", "上午先观察身体状态，热水、清淡早饭、整理桌面 10 分钟后再开始", "刑法学第 1 章：听众合法硕刑法课程 75-90 分钟，作为第 2 个切片，抓犯罪论总入口和构成要件", "刑法学第 1 章：已学内容小测 5-8 个点，只测今天听过的概念，不做整章章节题", "法理学第 2 章：用 20 分钟口头复述框架，卡住就看目录提示", dailyVocabularyTaskTitle, "饭后散步 15-20 分钟；晚上只做框架封口和提前睡"),
        daily("2026-07-05", "刑法1继续 + 民法1入口", "刑法学第 1 章：听众合法硕刑法课程 75-90 分钟，作为第 3 个切片，听完补构成要件小框架", "刑法学第 1 章：回看已学内容小测，补 3 个关键词，不做整章章节题", "民法第 1 章：听众合法硕民法课程 30-45 分钟，抓主体和民事法律关系入口", "民法第 1 章：只写 3-5 个入口关键词和 1 个不懂点，不急着做整章题", dailyVocabularyTaskTitle, "散步 15-20 分钟"),
        daily("2026-07-06", "刑法1切片 + 法理回背", "刑法学第 1 章：听众合法硕刑法课程 60-75 分钟，作为第 4 个切片，听完做已学内容小测", "刑法学第 1 章：整理一页犯罪论入口框架，不抄书，只写结构和易混点", "法理第 1-3 章：目录式回背 20 分钟", "背诵：法理第 2-3 章关键词，不追求一口气背完", dailyVocabularyTaskTitle, "散步或拉伸 20 分钟"),
        ...
        daily("2026-07-09", "法理6 + 民法1第2切片", "法理学第 6 章：目录式回忆并补框架", "民法第 1 章：听众合法硕民法课程 60-75 分钟，作为第 2 个切片，听完做已学内容小测", "背诵：刑法第 1 章已听关键词 10 分钟", "画民法第 1 章入口框架", dailyLongSentenceTaskTitle),
        ...
        daily("2026-07-11", "民法1继续 + 刑法1收口准备", "民法第 1 章：听众合法硕民法课程 60-75 分钟，作为第 3 个切片，听完做已学内容小测", "刑法学第 1 章：回看已听切片，列出整章章节题前还缺的知识点", "背诵：民法第 1 章关键词", "画民法法律关系框架", dailyVocabularyTaskTitle),
        ...
        daily("2026-07-16", "民法2第1切片 + 宪法1入口", "民法第 2 章：听众合法硕民法课程 60-90 分钟，作为第 1 个切片，听完做已学内容小测", "宪法学第 1 章：听众合法硕宪法课程 30-45 分钟，只抓总入口", "背诵：民法第 1 章第一轮关键词", "画宪法第 1 章入口框架", dailyVocabularyTaskTitle),
        daily("2026-07-17", "法理9 + 法制史1入口", "法理学第 9 章：错题回看、框架整理、第一轮背诵", "法制史第 1 章：听众合法硕法制史课程 30-45 分钟，抓朝代线索入口", "背诵：法理第 1-8 章第二次滚动", "整理法制史时间线入口图"),
        daily("2026-07-18", "刑法2第2切片 + 本周回炉", "刑法学第 2 章：听众合法硕刑法课程 60-90 分钟，作为第 2 个切片，听完做已学内容小测", "民法第 2 章：回看第 1 切片关键词 15 分钟", "背诵：刑法第 1 章第一轮", "错题：只回看刑法第 1 章已闭环错题"),
        daily("2026-07-19", "民法2第2切片 + 刑法2框架", "民法第 2 章：听众合法硕民法课程 60-90 分钟，作为第 2 个切片，听完做已学内容小测", "刑法第 2 章：整理听课小框架和不懂点", "背诵：民法第 1 章第一轮", "画民法第 1-2 章小框架"),
        ...
        daily("2026-07-23", "刑法3第2切片 + 民法3入口", "刑法学第 3 章：听众合法硕刑法课程 60-90 分钟，作为第 2 个切片，听完做已学内容小测", "民法第 3 章：听众合法硕民法课程 30-45 分钟，只抓入口", "背诵：刑法第 2 章第一轮关键词", "画刑法第 2-3 章框架"),
        ...
        daily("2026-07-25", "宪法1 + 法制史1入口闭环", "宪法学第 1 章：听众合法硕宪法课程 45-60 分钟，补入口框架并做已学内容小测", "法制史第 1 章：听众合法硕法制史课程 45-60 分钟，补时间线并做已学内容小测", "背诵：宪法/法制史第 1 章关键词", "画两科第 1 章框架"),
        daily("2026-07-26", "刑法3回炉 + 刑法4入口", "刑法学第 4 章：听众合法硕刑法课程 60-90 分钟，作为第 1 个切片，听完写 3 个关键词", "刑法第 3 章：回看已学内容小测，标出混淆点", "背诵：刑法第 2-3 章第一轮", "错题：只从刑法第 1-2 章已闭环范围挑 10 题"),
        daily("2026-07-27", "民法3回炉 + 民法4入口", "民法第 4 章：听众合法硕民法课程 60-90 分钟，作为第 1 个切片，听完写 3 个关键词", "民法第 3 章：回看已学内容小测，标出混淆点", "背诵：民法第 2-3 章第一轮", "画民法第 3-4 章框架"),
        daily("2026-07-28", "阶段复盘日", "法理第 1-13 章目录背诵", "刑法第 1-4 章：统计已听分钟数、未闭环切片和最常错 10 题", "民法第 1-4 章：统计已听分钟数、未闭环切片和最常错 10 题", "整理 7 月错题清单和背诵清单"),
        daily("2026-07-29", "法理闭环 + 刑法4第2切片", "法理学第 1-13 章：列出最不会背的 20 个点", "刑法学第 4 章：听众合法硕刑法课程 60-90 分钟，作为第 2 个切片，听完做已学内容小测", "法理学第 11-13 章：第一轮背诵关键词", "背诵：法理第 8-10 章滚动", "画法理 13 章总框架"),
        daily("2026-07-30", "民法4第2切片 + 宪法2入口", "民法第 4 章：听众合法硕民法课程 60-90 分钟，作为第 2 个切片，听完做已学内容小测", "宪法学第 2 章：听众合法硕宪法课程 30-45 分钟，只抓入口", "背诵：刑法第 3-4 章第一轮关键词", "画宪法第 1-2 章框架"),
    ...
    fun todaySchedule(date: LocalDate = LocalDate.now()): List<StudyScheduleBlock> {
        ...
        if (date == LocalDate.of(2026, 7, 3)) {
            return listOf(
                ...
                StudyScheduleBlock("19:10-19:35", "刑法加码", "如果身体还可以，只做刑法第 1 章已学内容小测和例题回忆；如果肚子还难受，这一块直接取消。"),
        ...
        val hasPartialQuiz = tasks.any { it.title.hasAny("已学内容小测", "例题回忆") }
        ...
        val practice = when {
            ...
            hasPartialQuiz -> "只测今天已经听过的内容：关键词、例题回忆、构成要件小框架；不抽后面没学的整章题。"
    ...
    fun todayTips(date: LocalDate = LocalDate.now()): List<StudyTip> {
        ...
        if (tasks.any { it.title.hasAny("看课", "听课") }) {
            tips += StudyTip(
                "看课别做抄写员",
                "课前看目录，课中只记定义、构成要件、易混点和例子；没听完整章前只做已学小测和例题回忆，整章闭环后再做章节题。",
            )
        }
        if (tasks.any { it.title.hasAny("做题", "题", "错题") }) {
            tips += StudyTip(
                "题目跟着闭环走",
                "半章阶段不要硬抽整章题。整章听完后再集中做章节题；每道错题只标一个主错因，第二天先看错因。",
            )
        }
    ...
    fun dynamicSchedulePrompt(
        ...
        return buildString {
            ...
            appendLine("- 刑法、民法新章优先排 60-90 分钟听课切片 + 3-5 个关键词 + 已学内容小测或小框架；恢复日才缩成 30-60 分钟入口切片。")
            appendLine("- 未听完整章前不要安排整章章节题，也不要机械安排“5 道题”；只做已学内容小测、例题回忆、关键词或小框架。整章课程闭环后，再安排章节题、错题整理和第一轮背诵。")
            appendLine("- 法理学不用重听大课，但必须背：先目录树和关键词第一轮，再进入规范表述和限时输出；不是只看目录。")
            appendLine("- 正常学习日不能只排一个短听课入口：至少安排 60-90 分钟专业课主块、30-45 分钟已学小测/题目/错题块、15-25 分钟背诵或框架封口。")
