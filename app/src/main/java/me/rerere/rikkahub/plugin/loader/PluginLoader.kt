...
class PluginLoader(
    ...
    private val pluginDispatcher = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "plugin-quickjs").apply { isDaemon = true }
    ...
    suspend fun loadPlugin(pluginInfo: PluginInfo): Result<LoadedPlugin> = withContext(pluginDispatcher) {
        try {
            ...
            sandbox.initialize()
            sandbox.injectConfig(resolveModelConfig(pluginInfo))

            val partsDir = pluginInfo.directory.resolve("parts")
            if (partsDir.isDirectory) {
                partsDir.listFiles { file -> file.isFile && file.extension.equals("js", ignoreCase = true) }
                    ?.sortedBy { it.name }
                    ?.forEach { part ->
                        Log.d(TAG, "Loading plugin script part ${pluginInfo.manifest.id}/${part.name}")
                        sandbox.evaluateFile(part)
                    }
            }
