...
private data class RoleplayRecord(
    val id: String,
    val scene: Int,
    val action: String,
...
private data class RoleplaySave(
    ...
    val records: List<RoleplayRecord> = emptyList(),
            ...
            for (index in 0 until array.length()) {
                ...
                add(
                    RoleplaySave(
                        ...
                        records = item.optJSONArray("records")?.toRoleplayRecords().orEmpty(),
            ...
            array.put(
                JSONObject().apply {
                    ...
                    put("records", JSONArray().apply {
                        save.records.takeLast(80).forEach { record ->
                            put(JSONObject().apply {
                                put("id", record.id)
                                put("scene", record.scene)
                                put("action", record.action)
                                put("narration", record.narration)
...
private fun JSONArray.toRoleplayRecords(): List<RoleplayRecord> = buildList {
    for (index in 0 until length()) {
        val item = optJSONObject(index) ?: continue
        add(
            RoleplayRecord(
                id = item.optString("id").ifBlank { "record-$index-${item.optLong("createdAt", 0L)}" },
                scene = item.optInt("scene", index + 1),
                action = item.optString("action"),
                narration = item.optString("narration"),
                roll = item.optInt("roll", 0),
...
private fun RoleplayRecord.summary(): String = "场景$scene：$action｜d20=$roll/$difficulty $outcome"

private fun splitNarrationForDisplay(text: String, targetLength: Int = 120, hardLimit: Int = 190): List<String> {
    if (text.isEmpty()) return emptyList()
    val result = mutableListOf<String>()
    val chunk = StringBuilder()
    text.forEach { char ->
        chunk.append(char)
        val sentenceEnd = char == '。' || char == '！' || char == '？' || char == '!' || char == '?' || char == '…' || char == '\n'
        if ((sentenceEnd && chunk.length >= targetLength) || chunk.length >= hardLimit) {
...
internal fun FormalRoleplayCampaignGame(
    ...
    Box(Modifier.fillMaxSize().background(Brush.verticalGradient(listOf(TrpgColors.backgroundTop, TrpgColors.backgroundBottom)))) {
        if (activeSave == null) {
            RoleplayArchive(saves, onExit, { activeSaveId = it.id }, { showCreator = true }, { deleteTarget = it })
        } else {
            RoleplayTable(
                activeSave,
    ...
    deleteTarget?.let { target ->
        AlertDialog(
            ...
            title = { Text("删除跑团存档？") },
            text = { Text("《${target.title}》的全部探索记录都会永久删除，无法恢复。") },
            confirmButton = {
                TextButton(onClick = {
...
private fun RoleplayArchive(
    ...
    LazyColumn(Modifier.fillMaxSize().navigationBarsPadding(), contentPadding = PaddingValues(18.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        if (saves.isEmpty()) {
            item { NeonPanel(TrpgColors.violet) { Text("暂无战役档案", color = TrpgColors.text, fontWeight = FontWeight.Bold) } }
        } else {
        ...
        text = {
            LazyColumn(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                items(ROLEPLAY_WORLDS, key = { it.id }) { world ->
                    Surface(Modifier.fillMaxWidth().clickable { selectedWorld = world }, RoundedCornerShape(16.dp), if (selectedWorld.id == world.id) world.accent.copy(alpha = 0.24f) else MaterialTheme.colorScheme.surfaceContainer) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        row.forEach { (id, name) ->
                            FilterChip(
                                ...
                                onClick = {
                                    selectedAssistants = if (id in selectedAssistants) selectedAssistants - id else if (selectedAssistants.size < 3) selectedAssistants + id else selectedAssistants
                                },
                                label = { Text(name) },
                            )
                        }
...
private fun RoleplayTable(
    ...
    var deleteRecordTarget by remember(save.id) { mutableStateOf<RoleplayRecord?>(null) }
    var confirmClearHistory by remember(save.id) { mutableStateOf(false) }

    fun hardDeleteRecord(record: RoleplayRecord) {
        val remaining = save.records.filterNot { it.id == record.id }
        onUpdate(
            save.copy(
                scene = (remaining.maxOfOrNull { it.scene } ?: 0) + 1,
                lastNarration = remaining.lastOrNull()?.narration ?: world.premise,
                log = remaining.map { it.summary() }.takeLast(80),
                records = remaining,
    ...
    fun runAction(cleanAction: String) {
        ...
        scope.launch {
            ...
            val retainedContext = if (save.records.isNotEmpty()) save.records.takeLast(5).joinToString("；") { it.summary() } else save.log.takeLast(5).joinToString("；")
            request(
                """
                    跑团世界：${world.title}
                    玩法标签：${world.system}
                ...
                val finalNarration = narration.ifBlank { "主持叙事暂时中断，但本次判定已经被记录。" }
                val record = RoleplayRecord(
                    id = "record-${System.currentTimeMillis()}-${save.scene}",
                    scene = save.scene,
                    action = cleanAction,
                ...
                val updatedRecords = (save.records + record).takeLast(80)
                val updated = save.copy(
                    ...
                    lastNarration = finalNarration,
                    log = updatedRecords.map { it.summary() },
    ...
    LazyColumn(Modifier.fillMaxSize().imePadding().navigationBarsPadding(), contentPadding = PaddingValues(16.dp, 14.dp, 16.dp, 28.dp), verticalArrangement = Arrangement.spacedBy(13.dp)) {
        ...
        item {
            NeonPanel(world.accent) {
                splitNarrationForDisplay(save.lastNarration).forEach { paragraph -> Text(paragraph, color = TrpgColors.text, style = MaterialTheme.typography.bodyLarge) }
            }
        }
        if (rolling) {
        ...
        if (save.records.isNotEmpty()) {
            item {
                NeonPanel {
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                        Text("探索记录", color = TrpgColors.text, fontWeight = FontWeight.Bold)
                        TextButton(onClick = { confirmClearHistory = true }) { Text("清空记录", color = TrpgColors.rose) }
                    }
                    save.records.takeLast(4).reversed().forEach { record ->
                        Surface(modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(14.dp), color = TrpgColors.backgroundTop.copy(alpha = 0.34f)) {
                            Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                                Text(record.summary(), color = TrpgColors.muted, style = MaterialTheme.typography.bodySmall)
                                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) { TextButton(onClick = { deleteRecordTarget = record }) { Text("删除", color = TrpgColors.rose) } }
                            }
                        }
                    }
                }
            ...
            item {
                NeonPanel {
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                        Text("旧版探索记录", color = TrpgColors.text, fontWeight = FontWeight.Bold)
                        TextButton(onClick = { confirmClearHistory = true }) { Text("清空记录", color = TrpgColors.rose) }
                    }
                    save.log.takeLast(4).reversed().forEach { Text("• $it", color = TrpgColors.muted, style = MaterialTheme.typography.bodySmall) }
                }
            }
        }
        item {
    ...
    deleteRecordTarget?.let { target ->
        AlertDialog(
            onDismissRequest = { deleteRecordTarget = null },
            title = { Text("永久删除这条记录？") },
            text = { Text("删除后，这条记录会从本地存档和后续模型上下文中移除，无法恢复。") },
            confirmButton = { TextButton(onClick = { hardDeleteRecord(target); deleteRecordTarget = null }) { Text("永久删除", color = MaterialTheme.colorScheme.error) } },
            dismissButton = { TextButton(onClick = { deleteRecordTarget = null }) { Text("取消") } },
        )
    }
    ...
    if (confirmClearHistory) {
        AlertDialog(
            ...
            title = { Text("清空全部探索记录？") },
            text = { Text("会永久清除这个存档里的场景记录、旁白和由这些记录累计的状态，并回到世界开场。此操作无法恢复。") },
            confirmButton = { TextButton(onClick = { hardClearHistory(); confirmClearHistory = false }) { Text("全部清空", color = MaterialTheme.colorScheme.error) } },
...
private fun NeonPanel(accent: Color = TrpgColors.border, content: @Composable ColumnScope.() -> Unit) {
    Surface(shape = RoundedCornerShape(20.dp), color = TrpgColors.panel.copy(alpha = 0.95f), border = BorderStroke(1.dp, accent.copy(alpha = 0.52f))) {
...
private fun MiniBadge(text: String, color: Color) {
    Surface(shape = RoundedCornerShape(50), color = color.copy(alpha = 0.16f)) {
        Text(text, Modifier.padding(horizontal = 8.dp, vertical = 5.dp), color = color, style = MaterialTheme.typography.labelSmall, fontWeight = FontWeight.Bold)
    }
}

@Composable
private fun StatusRune(label: String, value: Int, color: Color, modifier: Modifier = Modifier) {
    Surface(modifier, RoundedCornerShape(16.dp), color.copy(alpha = 0.18f), border = BorderStroke(1.dp, color.copy(alpha = 0.45f))) {
