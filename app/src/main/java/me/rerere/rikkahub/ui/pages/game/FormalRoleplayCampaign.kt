...
private data class RoleplayWorld(val id: String, val title: String, val system: String, val premise: String, val accent: Color)
...
@Composable
internal fun FormalRoleplayCampaignGame(request: CompanionNarrativeRequest, checkpoint: SharedGameCheckpoint) {
    ...
    Box(Modifier.fillMaxSize().background(if (activeSave == null) TrpgColors.archiveBrush else TrpgColors.tableBrush)) {
        if (activeSave == null) RoleplayArchive(saves, { activeSaveId = it.id }, { showCreator = true }, { deleteTarget = it })
        else RoleplayTable(
            activeSave,
            ROLEPLAY_WORLDS.firstOrNull { it.id == activeSave.worldId } ?: ROLEPLAY_WORLDS.first(),
            request,
            { activeSaveId = null },
            { changed -> persist(saves.map { if (it.id == changed.id) changed else it }) },
            checkpoint,
        )
    }

    if (showCreator) RoleplayCreator(
        settings.assistants.map { it.id.toString() to it.name.ifBlank { "未命名角色" } },
        { showCreator = false },
    ...
    deleteTarget?.let { target -> AlertDialog(
        onDismissRequest = { deleteTarget = null }, title = { Text("删除跑团存档？") },
...
@Composable
private fun RoleplayArchive(saves: List<RoleplaySave>, onOpen: (RoleplaySave) -> Unit, onCreate: () -> Unit, onDelete: (RoleplaySave) -> Unit) {
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(18.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
        item { Surface(shape = RoundedCornerShape(26.dp), color = TrpgColors.panel.copy(alpha = 0.92f)) {
            Column(Modifier.fillMaxWidth().padding(20.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) {
                ...
                Text("每个世界、队伍、骰运和秘密都会独立保存。", color = TrpgColors.muted)
            }
        } }
        item { Button(onClick = onCreate, modifier = Modifier.fillMaxWidth()) { Text("＋ 开启新战役") } }
        if (saves.isEmpty()) item { NeonPanel { Text("档案柜还没有记录。建立第一支队伍，让骰子决定故事。", color = TrpgColors.muted) } }
        else items(saves, key = { it.id }) { save ->
            val world = ROLEPLAY_WORLDS.firstOrNull { it.id == save.worldId } ?: ROLEPLAY_WORLDS.first()
            Surface(Modifier.fillMaxWidth().clickable { onOpen(save) }, RoundedCornerShape(22.dp), color = TrpgColors.panel, border = BorderStroke(1.dp, world.accent.copy(alpha = 0.5f))) {
                Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(9.dp)) {
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(7.dp)) {
                        MiniBadge("HP ${save.hp}", TrpgColors.rose); MiniBadge("SAN ${save.sanity}", TrpgColors.violet)
                        MiniBadge("LUCK ${save.luck}", TrpgColors.amber); MiniBadge("CLUE ${save.clues}", TrpgColors.cyan)
                    }
...
private fun RoleplayCreator(assistantChoices: List<Pair<String, String>>, onDismiss: () -> Unit, onCreate: (String, RoleplayWorld, List<String>) -> Unit) {
    var title by remember { mutableStateOf("") }; var selectedWorld by remember { mutableStateOf(ROLEPLAY_WORLDS.first()) }
    var selectedAssistants by remember { mutableStateOf(emptyList<String>()) }
    AlertDialog(
        onDismissRequest = onDismiss, title = { Text("建立新战役") },
        text = { LazyColumn(verticalArrangement = Arrangement.spacedBy(12.dp)) {
            item { OutlinedTextField(value = title, onValueChange = { title = it }, modifier = Modifier.fillMaxWidth(), label = { Text("存档名称（可留空）") }) }
            item { Text("选择世界", fontWeight = FontWeight.SemiBold) }
            items(ROLEPLAY_WORLDS) { world -> Surface(Modifier.fillMaxWidth().clickable { selectedWorld = world }, RoundedCornerShape(16.dp), color = if (selectedWorld.id == world.id) world.accent.copy(alpha = 0.24f) else MaterialTheme.colorScheme.surfaceContainer) {
            ...
            items(assistantChoices.chunked(3)) { row -> Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) { row.forEach { (id, name) -> FilterChip(
...
private fun RoleplayTable(save: RoleplaySave, world: RoleplayWorld, request: CompanionNarrativeRequest, onBack: () -> Unit, onUpdate: (RoleplaySave) -> Unit, checkpoint: SharedGameCheckpoint) {
    var action by remember(save.id) { mutableStateOf("") }; var busy by remember(save.id) { mutableStateOf(false) }
    var lastRoll by remember(save.id) { mutableIntStateOf(0) }; var lastDifficulty by remember(save.id) { mutableIntStateOf(0) }
    ...
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(13.dp)) {
        item { NeonPanel(world.accent) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) { TextButton(onClick = onBack) { Text("← 档案柜", color = TrpgColors.cyan) }; Text("SCENE ${save.scene}", color = world.accent, fontWeight = FontWeight.Bold) }
            Text(save.title, color = TrpgColors.text, style = MaterialTheme.typography.headlineSmall, fontWeight = FontWeight.Black)
            Text(world.system, color = TrpgColors.muted); Text(save.assistantNames.joinToString(" · "), color = TrpgColors.text)
        } }
        item { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            StatusRune("生命", save.hp, TrpgColors.rose, Modifier.weight(1f)); StatusRune("理智", save.sanity, TrpgColors.violet, Modifier.weight(1f)); StatusRune("幸运", save.luck, TrpgColors.amber, Modifier.weight(1f)); StatusRune("线索", save.clues, TrpgColors.cyan, Modifier.weight(1f))
        } }
        item { NeonPanel(world.accent) { Text("主持叙事", color = world.accent, fontWeight = FontWeight.Bold); Text(save.lastNarration, color = TrpgColors.text, style = MaterialTheme.typography.bodyLarge) } }
        if (save.log.isNotEmpty()) item { NeonPanel { Text("调查日志", color = TrpgColors.cyan, fontWeight = FontWeight.Bold); save.log.takeLast(5).reversed().forEach { Text("• $it", color = TrpgColors.muted, style = MaterialTheme.typography.bodySmall) } } }
        if (lastRoll > 0) item { Surface(shape = RoundedCornerShape(18.dp), color = if (lastOutcome.contains("成功")) TrpgColors.mint.copy(alpha = 0.16f) else TrpgColors.rose.copy(alpha = 0.16f)) {
            Row(Modifier.fillMaxWidth().padding(14.dp), horizontalArrangement = Arrangement.SpaceBetween) { Text("d20  $lastRoll / DC $lastDifficulty", color = TrpgColors.text, fontWeight = FontWeight.Bold); Text(lastOutcome, color = if (lastOutcome.contains("成功")) TrpgColors.mint else TrpgColors.rose, fontWeight = FontWeight.Bold) }
        } }
        item { NeonPanel(TrpgColors.violet) {
            OutlinedTextField(value = action, onValueChange = { action = it }, modifier = Modifier.fillMaxWidth(), label = { Text("你要做什么？") }, placeholder = { Text("调查、交涉、潜行、战斗，或者任何自由行动") }, minLines = 2, maxLines = 5)
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(9.dp)) {
                OutlinedButton(onClick = { if (save.luck > 0) onUpdate(save.copy(luck = save.luck - 1, updatedAt = System.currentTimeMillis())) }, enabled = lastRoll > 0 && save.luck > 0 && !busy, modifier = Modifier.weight(1f)) { Text("消耗幸运") }
                Button(onClick = {
                    val cleanAction = action.trim(); if (cleanAction.isBlank()) return@Button
                    val roll = Random.nextInt(1, 21); val difficulty = Random.nextInt(8, 16); val success = roll >= difficulty
                    lastRoll = roll; lastDifficulty = difficulty; lastOutcome = when { roll == 20 -> "大成功"; roll == 1 -> "大失败"; success -> "成功"; else -> "失败" }
                    busy = true; action = ""; val party = save.assistantNames.joinToString("、")
                    request("""
                        正式跑团世界：${world.title}
                        世界设定：${world.premise}
                        当前场景：${save.scene}
                    ...
                    """.trimIndent(), "你是严谨而富有戏剧张力的跑团主持人。必须接受骰子结果，描写环境变化、行动后果和至少一名同行角色符合人设的反应。不要替玩家决定下一步，不要直接完结。结尾给出2至3个可选行动，但允许玩家自由输入。控制在350至600字。") { narration ->
                        ...
                        onUpdate(save.copy(scene = save.scene + 1, hp = (save.hp - hpLoss).coerceAtLeast(0), sanity = (save.sanity - sanityLoss).coerceAtLeast(0), clues = save.clues + if (success) 1 else 0, lastNarration = narration.ifBlank { "判定已经发生，但主持叙事暂时中断。你仍可以继续行动。" }, log = (save.log + "场景${save.scene}：$cleanAction｜d20=$roll/$difficulty $lastOutcome").takeLast(80), updatedAt = System.currentTimeMillis()))
                ...
                }, enabled = action.isNotBlank() && !busy && save.hp > 0 && save.sanity > 0, modifier = Modifier.weight(1.3f)) { Text(if (busy) "主持人叙事中…" else "掷 d20 行动") }
            }
...
private fun NeonPanel(accent: Color = TrpgColors.border, content: @Composable ColumnScope.() -> Unit) {
    Surface(shape = RoundedCornerShape(20.dp), color = TrpgColors.panel.copy(alpha = 0.94f), border = BorderStroke(1.dp, accent.copy(alpha = 0.5f))) {
...
private fun MiniBadge(text: String, color: Color) {
    Surface(shape = RoundedCornerShape(50), color = color.copy(alpha = 0.16f)) { Text(text, Modifier.padding(horizontal = 8.dp, vertical = 5.dp), color = color, style = MaterialTheme.typography.labelSmall, fontWeight = FontWeight.Bold) }
}

@Composable
private fun StatusRune(label: String, value: Int, color: Color, modifier: Modifier = Modifier) {
    Surface(modifier, RoundedCornerShape(16.dp), color = color.copy(alpha = 0.18f), border = BorderStroke(1.dp, color.copy(alpha = 0.45f))) {
        Column(Modifier.padding(vertical = 11.dp), horizontalAlignment = Alignment.CenterHorizontally) { Text(value.toString(), color = TrpgColors.text, fontWeight = FontWeight.Black); Text(label, color = color, style = MaterialTheme.typography.labelSmall, fontWeight = FontWeight.Bold) }
    }
}
