...
object LivingJudgmentModelPlanner {
    ...
    fun buildPrompt(input: LivingJudgmentModelInput): String = buildString {
        ...
        appendLine("行动实现 Action 只执行判断结果：MESSAGE, WAIT, TOOL_USE, SET_ALARM, SCHEDULE_NEXT_PERCEPTION, ASK_USER, PASS。正式日记只通过 write_lulu_journal 工具保存；静默、等待和内部判断不自动生成辞海记录。")
        appendLine("状态生成在行动后发生，只生成 mood/bodyState/mindState/relationship/innerThought。innerThought 必须是角色第一人称没说出口的心理活动。")
        appendLine("辞海只保存挂心任务和工具写入的正式日记；正式日记记录角色真实感受和没说出口的想法，必须来自 write_lulu_journal 工具。挂心任务展示事件、目标、下次感知；不要把 belief、traitMotive、situationalMotive 当成用户可见状态栏。")
        appendLine("行动池只能从 MESSAGE, WAIT, TOOL_USE, SET_ALARM, SCHEDULE_NEXT_PERCEPTION, ASK_USER, PASS 中选；旧名 TOOL_CHECK 等同 TOOL_USE，旧名 JOURNAL_WRITE、WRITE_DIARY、READ、MEMORY_UPDATE 一律按 WAIT 处理，旧名 SCHEDULE_NEXT_TICK 等同 SCHEDULE_NEXT_PERCEPTION。")
    ...
    private fun String.normalizeActionNames(): String =
        replace(Regex("\\bTOOL_CHECK\\b", RegexOption.IGNORE_CASE), "TOOL_USE")
            ...
            .replace(Regex("\\bJOURNAL_WRITE\\b", RegexOption.IGNORE_CASE), "WAIT")
            .replace(Regex("\\bWRITE_DIARY\\b", RegexOption.IGNORE_CASE), "WAIT")
            .replace(Regex("\\bREAD\\b", RegexOption.IGNORE_CASE), "WAIT")
            .replace(Regex("\\bSCHEDULE_NEXT_TICK\\b", RegexOption.IGNORE_CASE), "SCHEDULE_NEXT_PERCEPTION")
            .replace(Regex("\\bMEMORY_UPDATE\\b", RegexOption.IGNORE_CASE), "WAIT")
