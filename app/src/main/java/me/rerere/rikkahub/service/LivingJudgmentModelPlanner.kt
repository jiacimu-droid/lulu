...
object LivingJudgmentModelPlanner {
    ...
    fun buildPrompt(input: LivingJudgmentModelInput): String = buildString {
        ...
        appendLine("行动实现 Action 只执行判断结果：MESSAGE, WAIT, TOOL_USE, SET_ALARM, WRITE_DIARY, SCHEDULE_NEXT_PERCEPTION, READ, ASK_USER, PASS。WRITE_DIARY 表示写入辞海日记：必须按角色人设、上下文、感知层、评估层、判断层生成 100-500 字第一人称真实想法；记忆提取不作为模型动作，辞海和聊天达到阈值后自动总结入向量记忆。")
