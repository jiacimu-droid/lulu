...
Claude Code must follow the same repository-wide rules as every other code agent. Read `AGENTS.md` before modifying code, then read `docs/architecture/CODE_INDEX.md`; use `docs/architecture/code-index.json` when exact symbols or file locations are needed. Shared product, safety, validation, workflow, and efficiency rules belong in `AGENTS.md` and must not be duplicated here.
